...
export default function UploadSlipPage() {
  ...
  if (done) {
    return (
      <main className="max-w-md mx-auto p-4 text-center bg-white border rounded-lg p-6 space-y-3">
        ...
        <p className="text-gray-600 mb-1">รหัสการจองของท่านคือ {bookingCode}</p>
  ...
  return (
    <main className="max-w-md mx-auto p-4">
      ...
      <p className="text-sm text-gray-500 mb-4">รหัสการจอง: {bookingCode}</p>

      <form onSubmit={submit} className="flex flex-col gap-3 bg-white p-4 rounded-xl shadow">
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium">เบอร์โทรศัพท์ที่ใช้จอง *</span>
          <input
            value={bookerPhone}
