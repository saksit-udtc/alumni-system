...
export default function UploadSlipPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/reservations/${bookingCode}/upload-slip`, {
        method: "POST",
        body: formData,
      });
    ...
    return (
      <div>
        ...
        <main className="max-w-md mx-auto p-6 text-center bg-white border border-cream-200 rounded-xl shadow-md space-y-3 mt-4">
          ...
          <p className="text-stone-600 mb-1">รหัสการจองของท่านคือ {bookingCode}</p>
          <p className="text-sm text-stone-500 mb-4">
            แอดมินจะตรวจสอบและยืนยันการชำระเงินโดยเร็วที่สุด ท่านสามารถตรวจสอบสถานะได้ที่หน้าสถานะการจอง
          </p>
          <button
  ...
  return (
    <div>
      ...
      <main className="max-w-md mx-auto p-4">
        ...
        <p className="text-sm text-stone-500 mb-4">รหัสการจอง: {bookingCode}</p>

        <form onSubmit={submit} className="flex flex-col gap-3 bg-white p-5 rounded-xl border border-cream-200 shadow-md">
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-stone-700">เบอร์โทรศัพท์ที่ใช้จอง *</span>
            <input
              value={bookerPhone}
